...
/*const logger = (request, respponse, next) => {
...
const authenticationToken = (request, response, next) => {
  let jwtToken
  const authHeadther = request.headers['authorization']
  if (authHeadther !== undefined) {
    jwtToken = authHeadther.split(' ')[1]
  }
  ...
  if (jwtToken === undefined) {
    ...
    jwt.verify(jwtToken, 'secreat_key', async (error, payload) => {
      if (error) {
        response.status(401)
...
// Register user Id
app.post('/user/', authenticationToken, async (request, response) => {
...
app.put('/districts/:districtId/', async (request, response) => {
  ...
  const updateQuery = `
    ...
    WHERE district_id=?;
  `
  const getUpdatedId = await db.run(updateQuery, [
    district_name,
    state_id,
    cases,
    cured,
